...
export const useGetAllUserWithdrawals = (filter: any) => {
    return useQuery({
...
export const useCreateUserWithdrawal = () => {
    ...
    return useMutation({
        ...
        onError: (error: any) => {
            toast.error(error.response?.data?.message ?? "Error creating user withdrawal request");
        },
    });
};
...
export const useUpdateUserWithdrawalStatus = () => {
    ...
    return useMutation({
        ...
        onError: (error: any) => {
            toast.error(error.response?.data?.message ?? "Error updating user withdrawal status");
        },
    });
};
